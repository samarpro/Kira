...
import { DAY_END_MIN, DAY_START_MIN, formatHm, getEntryTimeRange, isoFromDate } from "@/utils/schedule-time";
            ...
            {entries.map((entry) => {
                ...
                return (
                    <button
                        ...
                        className={cx(
                            "absolute right-1 left-1 overflow-hidden rounded-lg px-2 py-1 text-left shadow-xs ring-1 ring-inset transition-colors duration-100 ease-linear",
...
export function DayScheduleTimeline({ entries, day, className, onEntryClick }: DayScheduleTimelineProps) {
    ...
    return (
        <div className={cx("overflow-x-auto rounded-lg ring-1 ring-secondary", className)} aria-label={t("dashboard.dayTimelineAria")}>
            <div className="flex w-full min-w-[280px]">
                <TimelineRulerColumn />
                <DayColumn iso={iso} dayLabel={label} entries={entries} onEntryClick={onEntryClick} />
            </div>
        </div>
    );
}
