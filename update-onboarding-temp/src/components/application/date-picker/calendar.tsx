...
interface CalendarProps extends AriaCalendarProps<DateValue> {
    ...
    /** Dates that include at least one completed schedule entry (green cue). */
    completionHighlightedDates?: DateValue[];
    /** Fires when the user activates a day cell (click / keyboard). */
...
export const Calendar = ({ highlightedDates, completionHighlightedDates, onDayPress, className, children, ...props }: CalendarProps) => {
                            ...
                            <AriaCalendarGridBody className="[&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent [&_tr:last-of-type]:border-none">
                                {(date) => (
                                    <CalendarCell
                                        ...
                                        isCompletionHighlighted={completionHighlightedDates?.some((d) => date.compare(d) === 0)}
