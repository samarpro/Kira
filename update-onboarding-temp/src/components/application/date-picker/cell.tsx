...
import type { CalendarCellProps as AriaCalendarCellProps, DateValue } from "react-aria-components";
...
interface CalendarCellProps extends AriaCalendarCellProps {
    ...
    /** Extra cue: day has a completed task (green dot). */
    isCompletionHighlighted?: boolean;
    /** Whether to show out of range dates. */
    showOutOfRangeDates?: boolean;
    /** Called after the default cell press (e.g. add task on that day). */
            ...
            {({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, formattedDate }) => {
                ...
                return (
                    ...
                    >
                        ...
                        {(isHighlighted || isTodayDate || isCompletionHighlighted) && (
                            <div className="absolute bottom-1 left-1/2 flex -translate-x-1/2 gap-0.5">
                                ...
                                {isCompletionHighlighted && (
                                    <span className={cx("size-1.25 shrink-0 rounded-full bg-success-solid ring-1 ring-secondary", isDisabled && "opacity-50")} />
                                )}
                            </div>
                        )}
                    </div>
